...
export default function AssetOperationsPage() {
  ...
  const [resolveDamage, setResolveDamage] = useState({ id: "", repairCost: "1000000", repairTreatment: "EXPENSE", moneySourceCode: "", categoryCode: "REPAIR", supplierName: "", note: "" });
  ...
  return <ModuleFrame title="Vận hành tài sản" subtitle="GĐ3 - khấu hao, bảo trì, hư hỏng và sửa chữa" role={user?.role}>
    ...
    {active === "damage" && <div className="grid lg:grid-cols-[380px_1fr] gap-5">{canCreate && <form onSubmit={(e) => { e.preventDefault(); void send({ action: "REPORT_DAMAGE", assetId, ...damage }, "Đã gửi báo hỏng."); }} className="bg-white border border-slate-200 rounded-lg p-5 space-y-4 h-fit"><h2 className="font-bold">Báo cáo hư hỏng</h2><AssetSelect assets={data.assets} value={assetId} onChange={setAssetId} /><Field label="Mức độ"><select className="control" value={damage.severity} onChange={(e) => setDamage({ ...damage, severity: e.target.value })}><option value="LOW">Thấp</option><option value="MEDIUM">Trung bình</option><option value="HIGH">Nghiêm trọng</option></select></Field><Field label="Mô tả"><textarea className="control h-24 resize-none" value={damage.description} onChange={(e) => setDamage({ ...damage, description: e.target.value })} required /></Field><button className="primary-button w-full">Gửi báo hỏng</button></form>}<div className="space-y-4">{resolveDamage.id && <form onSubmit={(e) => { e.preventDefault(); void send({ action: "RESOLVE_DAMAGE", ...resolveDamage, resolvedAt: new Date().toISOString() }, resolveDamage.repairTreatment === "EXPENSE" ? "Đã xử lý và tạo phiếu chi chờ duyệt." : "Đã hoàn tất xử lý báo hỏng."); setResolveDamage({ id: "", repairCost: "1000000", repairTreatment: "EXPENSE", moneySourceCode: "", categoryCode: "REPAIR", supplierName: "", note: "" }); }} className="bg-blue-50 border border-blue-100 rounded-lg p-4 grid md:grid-cols-3 gap-3 items-end"><Field label="Cách xử lý"><select className="control" value={resolveDamage.repairTreatment} onChange={(e) => setResolveDamage({ ...resolveDamage, repairTreatment: e.target.value })}><option value="EXPENSE">Chi phí ngay</option><option value="ALLOCATE">Phân bổ nhiều kỳ</option><option value="CAPITALIZE">Tăng nguyên giá</option></select></Field><Field label="Chi phí"><input type="number" className="control" value={resolveDamage.repairCost} onChange={(e) => setResolveDamage({ ...resolveDamage, repairCost: e.target.value })} /></Field><Field label="Nguồn tiền"><input className="control" value={resolveDamage.moneySourceCode} onChange={(e) => setResolveDamage({ ...resolveDamage, moneySourceCode: e.target.value })} placeholder="Trống = tự chọn" /></Field><Field label="Nhóm chi phí"><input className="control" value={resolveDamage.categoryCode} onChange={(e) => setResolveDamage({ ...resolveDamage, categoryCode: e.target.value })} /></Field><Field label="Nhà cung cấp"><input className="control" value={resolveDamage.supplierName} onChange={(e) => setResolveDamage({ ...resolveDamage, supplierName: e.target.value })} /></Field><div className="flex gap-2"><button className="primary-button flex-1">Lưu xử lý</button><button type="button" onClick={() => setResolveDamage({ id: "", repairCost: "1000000", repairTreatment: "EXPENSE", moneySourceCode: "", categoryCode: "REPAIR", supplierName: "", note: "" })} className="secondary-button">Hủy</button></div></form>}<section className="table-panel"><Panel title="Phiếu báo hỏng" reload={loadData} /><Table headers={[{ label: "Phiếu" }, { label: "Tài sản" }, { label: "Mức độ" }, { label: "Mô tả" }, { label: "Xử lý" }, { label: "Thao tác", align: "right" }]}>{data.damageReports.map((row) => <tr key={row.id} className="border-t border-slate-100"><Cell><b>{row.code}</b><small>{row.status}</small></Cell><Cell>{row.asset.code} - {row.asset.name}</Cell><Cell><span className="status bg-amber-50 text-amber-700">{row.severity}</span></Cell><Cell>{row.description}</Cell><Cell>{row.repairTreatment || "-"}<small>{row.repairCost ? `${money(row.repairCost)} đ` : ""}</small></Cell><Cell right>{canEdit && row.status !== "COMPLETED" && <button className="action-link text-blue-700" onClick={() => setResolveDamage({ ...resolveDamage, id: row.id, supplierName: row.asset.name })}>Xử lý</button>}</Cell></tr>)}</Table></section></div></div>}
  </ModuleFrame>;
}
