...
export const importTemplates: ImportTemplateDefinition[] = [
  ...
  {
    code: "PAYROLL_DEPARTMENT_V1",
    importType: "PAYROLL",
    name: "Bảng lương theo bộ phận",
    description: "Template lương gom theo cửa hàng và bộ phận (không khai từng nhân viên): TỔNG CHI PHÍ CÔNG TY lên chi phí nhân sự, LƯƠNG THỰC NHẬN thành công nợ phải trả người lao động, hai cột bảo hiểm thành công nợ phải trả cơ quan BHXH.",
    fields: [
      { field: "period", label: "Kỳ lương", required: true, type: "text", aliases: ["ky luong", "ky", "period", "payroll period"] },
      { field: "branch_code", label: "Cửa hàng", required: true, type: "text", aliases: ["chi nhanh", "branch", "store"] },
      { field: "department_code", label: "Phòng ban", required: true, type: "text", aliases: ["phong ban", "bo phan", "department"] },
      { field: "headcount", label: "Số lượng nhân sự", required: false, type: "integer", aliases: ["so luong nhan su", "so nhan su", "so luong nhan vien", "headcount"] },
      {
        ...
        required: false,
        type: "number",
        aliases: ["tong luong/thang", "tong luong thang", "luong co ban", "tong luong"],
        note: "Lương tháng theo hợp đồng, chỉ để tham chiếu — KHÔNG cộng vào TỔNG CHI PHÍ CÔNG TY",
      },
      { field: "hourly_salary", label: "TỔNG LƯƠNG THEO GIỜ CÔNG", required: false, type: "number", aliases: ["tong luong theo gio cong", "luong theo gio cong", "luong gio cong"] },
      { field: "meal_allowance", label: "Phụ cấp tiền cơm", required: false, type: "number", aliases: ["phu cap tien com", "tien com", "phu cap com"] },
      { field: "parking_allowance", label: "Phụ cấp gửi xe", required: false, type: "number", aliases: ["phu cap gui xe", "gui xe", "phu cap xe"] },
      { field: "svc_amount", label: "SVC", required: false, type: "number", aliases: ["svc", "phu thu svc"] },
      { field: "kpi_amount", label: "KPI", required: false, type: "number", aliases: ["kpi", "thuong kpi"] },
      { field: "other_allowance", label: "Phụ cấp khác", required: false, type: "number", aliases: ["phu cap khac", "phu cap con lai"] },
      {
        field: "company_insurance",
        label: "Bảo hiểm (công ty chịu)",
        required: false,
        type: "number",
        aliases: ["bao hiem (cong ty chiu)", "bao hiem cong ty chiu", "bao hiem cty chiu", "bao hiem cong ty", "bao hiem"],
        note: "Phần bảo hiểm công ty đóng: CÓ cộng vào TỔNG CHI PHÍ CÔNG TY",
      },
      {
        field: "mandatory_insurance",
        label: "Bảo hiểm bắt buộc",
        required: false,
        type: "number",
        aliases: ["bao hiem bat buoc", "bhbb", "bao hiem nguoi lao dong", "bao hiem nld", "bao hiem nhan vien"],
        note: "Phần bảo hiểm trừ vào lương người lao động: KHÔNG cộng vào TỔNG CHI PHÍ CÔNG TY, nhưng cộng với cột bên trái thành khoản phải trả cơ quan BHXH",
      },
      {
        field: "total_company_cost",
  ...
  },
  {
    ...
    fields: [
      ...
      // Không có cột Đơn giá (chị Bình 26/09/2026): phân bổ luôn ra số lẻ, số chuẩn là Tổng giá
  ...
  },
  {
    ...
    fields: [
      ...
      { field: "vat_rate", label: "Thue suat GTGT", required: false, type: "text", aliases: ["thue suat gtgt", "thue suat thue gtgt", "thue suat thue gia tri gia tang", "thue suat", "thue gtgt", "vat", "vat rate", "thue"], note: "KKKNT, 0%, 5%, 8% hoac 10%. KKKNT = khong ke khai nop thue (khac 0%: cung 0 dong thue nhung khac dong to khai). De trong = KKKNT (khong co thue). Chi dung cho phieu nhap." },
      { field: "amount_before_tax", label: "Thanh tien truoc thue", required: false, type: "number", aliases: ["thanh tien truoc thue", "thanh tien chua thue", "tien truoc thue", "amount before tax", "subtotal"], note: "= So luong x Don gia, lam tron toi dong. De trong thi he thong tu tinh; co khai thi he thong doi chieu, lech qua 1 dong la bao loi ngay o buoc xem truoc." },
      { field: "amount_after_tax", label: "Thanh tien sau thue", required: false, type: "number", aliases: ["thanh tien sau thue", "thanh tien co thue", "tien sau thue", "tong tien sau thue", "amount after tax", "total"], note: "= Thanh tien truoc thue x (1 + thue suat), lam tron toi dong. Day la so PHAI TRA NCC: cong no phai tra lay so nay, con gia von ton kho van lay so truoc thue. Khai theo SO TREN HOA DON: lech vai dong voi so tu tinh (NCC tron khac) thi he thong lay so cua ban; lech qua 1.000 d hoac 1% moi bao loi." },
